/*
 * MOONStroks.js Ver.1.9.0 (2013/12/23) by fintopo
...
(function(){
  ...
  var min = MOONStrokes.min = function(arr, callback){
    var ret = reduce(arr, function(memo, value, i){
      var v = callback(value, i);
      if (v < memo.computed) {
        memo.computed = v;
        memo.value = value;
  ...
  Stroke.prototype.rough = function(cycle_x, cycle_y, power_x, power_y, gap_x, gap_y){
    // ストロークの凸凹化処理
    // 現在のストロークに対して周期と乱数で位置を変更する
    // power_x, power_y: 移動の強さ
    // cycle_x, cycle_y: 大きさに対する周期。
    // gap_x, gap_y: 乱数の強さ
    var _this = this;
    //
    var cx = cycle_x / 10;
    ...
    var gx = gap_x / 10;
    var gy = gap_y / 10;
    var w = _this.right() - _this.left();
    var h = _this.bottom() - _this.top();
    var x0 = 0;
    var y0 = 0;
    _this.each(function(point){
      x0 += (Math.random() - 0.5) * gx;
      y0 += (Math.random() - 0.5) * gy;
      var x = sin(cy * 360 * (point.y + y0) / h) * px;
      var y = sin(cx * 360 * (point.x + x0) / w) * py;
      point.moveTo(x, y);
    });
    //
    return this;
  ...
  var Paper = MOONStrokes.Paper = function(id, DEBUG) {
    this.id = id;
    this.info = (DEBUG) ? DEBUG : (id) ? MOON.getPaperJSON(this.id) : {
      "version": "0.2"
      ,"x":0
      ,"y":0
      ,"width":768
